    ...
    ({ pkgs, ... }: {
      ...
      home.packages = [
        (pkgs.writeShellScriptBin "ubuild" ''
          nix build --extra-experimental-features nix-command --extra-experimental-features flakes ''${UNIVERSE_PATH:-$HOME/.universe}#homeConfigurations.${hostname}.activationPackage "$@"
        '')
        (pkgs.writeShellScriptBin "uswitch" ''
          nix build --extra-experimental-features nix-command --extra-experimental-features flakes ''${UNIVERSE_PATH:-$HOME/.universe}#homeConfigurations.${hostname}.activationPackage "$@" \
            && "$(nix path-info --extra-experimental-features nix-command --extra-experimental-features flakes ''${UNIVERSE_PATH:-$HOME/.universe}#homeConfigurations.${hostname}.activationPackage)"/activate
        '')
      ];
    })
  ];
